...
// Import dari file baru kita
import ResultCardModel1 from "./ResultCardModel1";
import { loadFashionModel, predictFashion } from "../utils/tfjsUtils";

const PREPROCESS_MODE = "raw";

export default function Model1Local() {
  ...
  useEffect(() => {
    async function initModel() {
      try {
        ...
        console.error(err);
        setError("Model gagal dimuat. Cek folder public/models/fashion.");
      } finally {
  ...
  async function handlePredict() {
    ...
    try {
      ...
      console.error(err);
      setError("Prediksi gagal. Cek apakah preprocessing sesuai.");
    } finally {
      setPredicting(false);
    }
  }
  ...
  return (
    <section className="relative overflow-hidden rounded-xl border border-[#e6d5a8] bg-[#fffaeb] p-5 shadow-sm md:p-8">
      ...
      <div className="mb-8 grid gap-6 lg:grid-cols-[1fr_0.6fr] lg:items-end">
        <div>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#e6d5a8] bg-white px-4 py-2 text-sm font-medium text-[#fa520f]">
            <Brain size={16} /> CNN Prediction System (TensorFlow.js)
          </div>
          ...
          <p className="mt-4 max-w-2xl leading-7 text-[#4a4a4a]">
            Menggunakan model TensorFlow.js yang dieksekusi langsung di memori browser klien.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-[#e6d5a8] bg-white p-5">
          <div className="flex items-center justify-between">
            ...
            <span className={loadingModel ? "text-[#ffa110]" : error ? "text-red-600" : "text-emerald-600"}>
              {loadingModel ? "Loading..." : error ? "Error" : "Ready"}
            </span>
          </div>
          <div className="mt-3 flex items-center justify-between">
            <span className="text-sm text-[#6a6a6a]">Input Size</span>
            <span className="font-semibold text-[#1f1f1f]">{inputSize} × {inputSize}</span>
          </div>
          <div className="mt-3 flex items-center justify-between">
            <span className="text-sm text-[#6a6a6a]">Classes</span>
            <span className="font-semibold text-[#1f1f1f]">{classNames.length || "-"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          ...
          <label className="flex min-h-80 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#c7c7c7] bg-[#fafafa] p-6 text-center transition hover:border-[#fa520f]">
            {!preview ? (
              <>
                ...
                <p className="font-semibold text-[#1f1f1f]">Klik untuk upload gambar</p>
              </>
            ) : (
              <img
                ref={imageRef}
        ...
        {/* Memanggil ResultCard Khusus Model 1 */}
        <ResultCardModel1 result={result} />
      </div>
    </section>
  );
}